...
public partial class DateNodeViewModel : ViewModelBase
{
    ...
    /// <summary>
    /// Years, then months, then days, in the grid's order. A year opens if it's the only one, and a
    /// month if it's its year's only one, so a single holiday shows its days straight away.
    /// </summary>
    public static IReadOnlyList<DateNodeViewModel> Build(IReadOnlyList<DayHeaderViewModel> days)
    {
        ...
        foreach (var year in days.Where(d => d.Day is not null).GroupBy(d => d.Day!.Value.Year))
        {
